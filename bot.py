...
from config import SECRET, WEBAPP_HOST, WEBAPP_PORT, WEBHOOK_PATH, WEBHOOK_URL
    ...
    for topic in await fast_api_client.get("topics"):
        markup.add(
            types.InlineKeyboardButton(
                topic, callback_data=topics_cb.new(id=topic, action="learn")
            ),
        )
    return markup
...
async def query_learn(
    ...
    text = (
        f"Nice choice, {query.from_user.first_name}! "
